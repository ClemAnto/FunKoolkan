...
## FunKoolkan — Prospettiva (DUE modelli con flag) + Launcher  *(2026-06-21, riprogettato 2026-06-28)*

**`assets/scripts/config/Perspective.ts` — DUE modelli intercambiabili via la costante `PERSPECTIVE_MODEL` (`PerspectiveModel.Perspective | .Tilt | .Flat`).** Fisica SEMPRE piatta in ground space `[−W/2,W/2]×[0,D]`; cambia solo la mappa di render. **Entrambi tassellano ESATTO in V e H** (overlap 0.0000 a ogni profondità — verificato) e sono invertibili (aiming esatto).
- **Perspective (A)** = vera omografia 1-punto (default, mantiene il pavimento-trapezio attuale). DUE knob: `PERSPECTIVE_FAR_SCALE` (sFar, **0.5** in play; più basso = più convergenza X) **+ `PERSPECTIVE_NEAR_SCALE` (sNear, "offset arena", 0.7 in play)** = scala verticale al bordo BASSO. `aX=1/sFar−1`; `_u0=(1/sNear−1)/aX` (offset frustum del bordo vicino, 0 se sNear=1); `_denomY=sNear−sFar`; `_KY=sFar·aX·(1−_u0)/_denomY` (=1 se sNear=1); `D=H/sFar`. `depthScale s=1/(1+aX·t)` (X, near-scale resta 1); `_sYof`=scala Y con offset; `projectX=xp·s`; `projectY=H·(sNear−sY)/_denomY`; `sizeXFactor=s`; `sizeYFactor=_KY·sY²`. **`sNear` tara lo schiacciamento del bordo basso** (1.0=tondo come prima; ↓=più piatto; **≈√sFar = uniforme sopra↔sotto**; clamp >sFar, oltre serve abbassare sFar). `sNear=1` riproduce **bit-identico** il vecchio modello (`_KY=1`, sizeY=s², squash 1.0→sFar). L'offset NON tocca la X (trapezio invariato).
- **Tilt (B)** = ortografico puro ~60° (richiede pavimento + bounds **RETTANGOLARI**, niente convergenza X). Knob `TILT_FORESHORTEN` (k, **0.5** = "dimezza la Y"). `D=H/k`; `projectX=xp`; `projectY=k·yp`; `sizeXFactor=1`; `sizeYFactor=k`. Squash **costante** 0.5 ovunque, **nessun** rimpicciolimento con la profondità (la dà l'arte + DepthSort). Mappa affine → inversa banale.
- **Tassellazione esatta = la coppia `projectYCenter(yp,r)` (centro = midpoint dell'estensione verticale proiettata) + `vStackFactor(yp,r)` (altezza sprite = proiezione del DIAMETRO).** Telescopia: gap-tra-centri == somma delle due semi-altezze, a ogni profondità, in entrambi i modelli. **Le rune (Stone) usano queste**, NON `projectY`/`sizeYFactor`. `floorTilt(yp)=sizeY/sizeX` = aspetto del disco a terra (per ellissi/dot/drag-cone; A→s, B→k).
- `configurePerspective(W,H)` chiamato da `FitScale.onLoad` + `ArenaBounds.rebuild`; rilegge `PERSPECTIVE_MODEL`. `unprojectX/Y` = inverse per-modello (guard clamp, no NaN).
- **Ground space**: `ArenaBounds` costruisce il rim **direttamente in ground space** (i muri SONO quei punti); `boundaryPhysics` = rim ground; debug + `boundaryImage` = rim **proiettato avanti** → trapezio (A) o rettangolo (B).
- ⚠️ **`PERSPECTIVE_ENABLED`/`PERSPECTIVE_Y_FORESHORTEN`/`PERSPECTIVE_Y_SCALE` RIMOSSE** (sostituite dall'enum + vStackFactor). Il vecchio `GROUND_TILT=0.5` hardcoded in Stone/StoneLauncher/Glue/ManaFlame/AkuAku → ora `floorTilt(yp)`/`sizeYFactor(yp)` (mode-aware).

**Stone** (`entities/Stone.ts`): posizione via `projectX` + **`projectYCenter(y,r)`**; scala vista `(ws·vs·sizeXFactor, ws·vs·vStackFactor(y,r))` (anisotropa, **tassella esatto**); corpo ruota (`fixedRotation=false`); `rotationNode.angle = this._zAngleDeg()` → angolo Z pieno **±180** ricavato dal **quaternione** del corpo con `atan2(2(wz+xy), 1−2(y²+z²))·180/π` (NON `this.node.angle`, che ripiega a ±90 — vedi gotcha). Stesso helper usato dal gizmo di debug.

**StoneLauncher** (`entities/StoneLauncher.ts`): slingshot **ancorato al nodo launcher** (spawn + traiettoria + direzione derivati dalla sua posizione). Spawn = `unprojectX/Y(launcher pos)`; velocità via `_groundDir(eff)` (Jacobian dell'inversa con passo ε, perché l'omografia accoppia X/Y); traiettoria simulata in ground space su `boundaryPhysics` inset di `stoneRadius`; pallini disegnati **piatti** (ellisse `floorTilt(yp)`, mode-aware); drag-metric `_dragLen` un-squasha la Y col `floorTilt` al launcher.
...
**`EditMode.ts`** (sul nodo GameManager): tool dev per **trascinare le stone in arena**. `toggle()` si **auto-aggancia** a `editButton.on(Button.EventType.CLICK)` (niente ClickEvent manuale). Drag = stone più vicina in ground space (`Stone.all`, dist ≤ `radius+8`) → corpo **Kinematic** → `setPosition` al dito → **Dynamic** + vel 0 al rilascio. Stato attivo = **tinta dello Sprite** del bottone (Button transition SCALE non tocca il colore). Il launcher resta attivo in EDIT (sospeso solo durante il drag). Wiring: `launcher`, `editButton`, `arena`(opz, fallback `launcher.arena`).

## Edit tools + debug toggle + gemType  *(2026-06-23, v0.2.1)*

- **`CurlingScorer` — trigger "ferma sul TEE"**: corpo **Dynamic** con velocità < `restSpeed` mantenuta `restDelay`s (Kinematic del drag ignorato). **Calibrazione: `restSpeed = 10`** (in scena; default codice 10). Era **25 → troppo generoso**: una stone che scivola ancora a 15-24 contava come "ferma". `restSpeed` è la soglia di "parcheggiata", NON di "lenta" → tienila bassa. (Un gate "doveva muoversi >restSpeed sul tee" è stato provato e **rimosso**: la stone arriva al tee già più lenta → non scattava mai.)
- **Toggle debug globale**: `config/DebugDraw.ts` = flag unico persistito in localStorage (`fk.debugDraw`), pilotato da `DebugToggle.ts` sul bottone DEBUG. Ogni overlay fa `flagLocale || DebugDraw.enabled`. **`ArenaBounds.showDebugOutline` reso reattivo in `update()`** (prima disegnava SOLO a `start()` → un toggle a runtime non lo raggiungeva).
- **`Rune.gemType`** = `@property` enum `RuneKind` (`RuneTypes.ts`) + `@executeInEditMode` (anteprima live). **`gemType` È l'indice in `gems`** (`gems[gemType]`), non un campo separato; `setType()` resta l'API runtime.
- **`EditPanel.ts`** (nodo EditPanel in HUD): palette di rune → **drag&drop su arena spawna una stone** di quel tipo (ghost che segue il dito; spawn via `StoneLauncher.trySpawnAtUI`→`spawnRestingStone(gx,gy,type)`, stessa config del launcher). **SAVE/LOAD** layout arena in localStorage (`fk.arenaLayout`, `[{t,x,y}]`); LOAD svuota e ricrea **al frame dopo** (`scheduleOnce(…,0)`) per non spawnare dentro corpi Box2D in distruzione.
- **`config/EditState.ts`** (stato condiviso `editing`+`panelRect`): `EditMode` scrive `editing` su `toggle()`; `EditPanel` (**nodo sempre attivo**) lo legge in `update()` e **si mostra/nasconde da solo** (Sprite bg disabilitato + figli disattivati). Grab-skip di EditMode usa `EditState.panelRect`.
- ⚠️ **GOTCHA (costato 3 giri)**: le reference `@property` scritte **a mano nel `.scene`** (`"x":{"__id__":N}`) **vengono azzerate dall'editor** al risalvataggio (e i `__id__` shiftano). → assegnare dall'**inspector**, oppure evitare la reference con un **modulo di stato condiviso** (come `EditState`/`DebugDraw`).

---


## Aku preghiera — inner glow + bollicine  *(2026-06-27, v0.2.8)*

- **Inner glow (NON outer)**: l'outer glow (alone che esce dalla sagoma) **clippa al quad** dello sprite e servirebbe padding texture (deep-research → memoria `reference_cocos_outer_glow`). Scelto il **GLOW INTERNO**: `SpriteFlash.effect` blocco glow fa **EROSIONE** (per ogni pixel opaco campiona la *trasparenza* dei vicini ad anelli, pesata per vicinanza → rim sul bordo interno che sfuma verso il centro). Resta **dentro il quad → zero padding**. Gated su `glowColor.a>0` (sprite senza glow saltano il loop). Uniform aggiunti all'UBO `FlashConstant`: `glowColor` (.rgb colore, .a intensità) + `glowParams` (.xy raggio-rim in UV per-asse, .z falloff, .w qualità 0–3). **Loop a bound costanti (MAX 5 anelli × 16 dir) con `break` dinamico** = WebGL-safe; qualità→(anelli×dir): 0=2×6, 1=3×8, 2=4×12, 3=5×16. **Default Low (perf-first**, precisione non critica).
- **`Glow.ts`** (`VFX/Glow`, riusabile): drop su qualsiasi Sprite col material **`SpriteFlash`**; pilota `glowColor`/`glowParams` via material instance (gather lazy, no-op se il material non ha `glowColor` → `passes[0].getHandle`). `intensity` 0–1 + `width`(px rim INWARD)/`falloff`/`quality`/`color`. `@executeInEditMode` → preview live (ri-applica ogni tick in editor; a runtime ri-prova finché risolve il material poi cache). `pulse()/fadeTo()/refresh()/attach()`. ⚠️ **NON** metterlo sull'Aku: l'Aku pilota già `glowColor` da codice → conflitto.
- **Aku glow** (in `AkuAku`): `_setGlow(on)` setta `glowParams` (raggio = `GLOW_WIDTH_PX 12`/texSize variante) + colore **viola `170,80,255`**; `_tickGlow` pulsa `.a` tra `GLOW_PULSE_MIN .25`–`MAX .9` (~1.6s) **solo in `Phase.Pray`**. Spento uscendo/colpito/morte/reset. ⚠️ `hit()` salta `_enter` → glow/bolle spenti **dentro `hit()`** (e ri-armati se torna a Pray).
- **Bollicine** `assets/resources/prefabs/PrayerBubbles.prefab` (`ParticleSystem2D` custom, UUID `4cd9765c-…`): up (angle 90°/var18, `gravity y+25`, speed 22, `tangentialAccel 8` wobble), viola/lavanda con fade-out alpha, **blend alpha** (src 2 / dst 4), `playOnLoad:false`, `duration -1`, **entrata zoom-in** (`startSize 2 → endSize 16`; PS2D interpola **lineare/monotòno** → impossibile fade-in+fade-out nella stessa particella senza il particellare **3D**), texture provvisoria `aura.png` (`edbf98bd…@f9941`), `positionType GROUPED`. **Auto-attaccato** da `AkuAku._setBubbles` via `resources.load('prefabs/PrayerBubbles')` + `instantiate` figlio della root (come VFX morte, no @property); `resetSystem()`/`stopSystem()`.
- ⚠️ **GOTCHA — prefab creato a mano**: un `.prefab`+`.meta` scritti da editor esterno **vanno reimportati** dall'editor (Reimport cartella `assets` / riavvio) prima che `resources.load` li trovi, altrimenti load fallisce in silenzio. Log diagnostici in `AkuAku` (`failed to load VFX prefab` / `prayer bubbles attached: ps=… frame=…`). **Punto aperto al 2026-06-27**: in Play "non vedo le bolle" → verificare reimport + leggere quei log (dettagli in CLAUDE.md, blocco sessione 2026-06-27 bis).


## Prototipo "blob + FIAMMA DI MANA" + prospettiva  *(2026-06-28, dietro `GameMode.stickyPrototype`)*

**Glue sticky (blob)** — `assets/scripts/entities/Glue.ts`, ramo `_updateBondsSticky`:
- `STICKY_COHESION = 400`, `STICKY_BONDED_DAMPING = 8` (≈ smorzamento critico). La molla `cohesion=30` ereditata era troppo debole per le rune pesanti (densità 8, massa Box2D alta) → il joint **non si contraeva** (creep ~0.9 u/s, sembrava fermo). 400 contrae in ~0.5–0.8s.
- `STICKY_BOND_MIN = 1.0` → rest del bond a **2r** (= collider che si toccano). ⚠️ A `0.9` (rest DENTRO il footprint-collider) la molla spinge oltre il collider e il collider respinge → **vibrazione** (peggiore con cohesion alta). Il footprint-collider è un pavimento rigido: per scendere sotto 2r servirebbe disattivare la collisione rune-rune (collision matrix), non toccare il collider.
- `STICKY_MAGNET_FORCE = 0` → **magnetismo OFF** (per testare il gameplay senza). A >0 = pull gentile verso le vicine non a contatto (fattore × distanza, scala come `cohesion`).
- `House`/`TeeBeacon`/`HouseBeacon`/`Petrifier`/`CurlingScorer`/`RoundManager`/`AkuAkuSpawner` = **no-op in prototipo** (early-return su `GameMode.stickyPrototype`).

**ManaFlame** — `assets/scripts/entities/ManaFlame.ts` (auto-installata dal `GameManager`, trova tutto via `StoneLauncher.instance`):
- Cadenza **ogni 5 colpi** (`SPAWN_EVERY=5`, conta `StoneLauncher.launchCount`), spawn **a fine tiro** (transizione arena moving→still, soglia `MOVE_THRESHOLD=40`). Vita `LIFE_SHOTS=3` colpi inutilizzata → despawn con **flash bianco + pop-out** (pop-in all'apparire).
- **Area di contatto = derivata dallo sprite** (`_areaRadius = spriteHalf·FLAME_VIEW_SCALE·AREA_FACTOR`, misurata alla prima view); detection = overlap del **footprint proiettato** (come `House`/tee) **swept** (nessun limite di velocità, ricostruisce il segmento dal moto) + entry-guard `_exempt`; **solo `StoneLauncher.lastFired`** si accende; non spawna sotto una runa. Debug **magenta**.
- VFX = prefab autorato `resources/prefabs/ManaFlame.prefab` (particellare + `assets/animations/flame.anim` avviati in auto: `resetSystem()` + `Animation.play()`). Apertura round 1 = 3 grappoli separati di 3 rune (3 tipi) sin/centro/des (centro = colore caricato) + fiamma al centro.
- **Overpower** (`Overpower.ts`, aggiunto a runtime quando la runa attraversa la fiamma): `ignite()` = boost `INITIAL_BOOST=35` lungo la direzione + un altro a ogni rimbalzo (dimezzante, via contact-listener); a riposo `_detonate` = catena `ManaLightning` runa→runa sul cluster contiguo stesso-colore → ogni runa → **`RaisingStar.launchAtKoolkan`** (stella su Koolkan) o `shatter`. Off-colore → `_fizzle` (fade). `RaisingStar` generalizzata (cubo OPPURE `Koolkan.instance`); accessor statici `Koolkan.instance`/`RaisingStar.instance`/`StoneLauncher.instance`/`.lastFired`/`.launchCount`.

**Prospettiva — schiacciamento del bordo basso tarabile (RISOLTO 2026-06-29)**  *(`Perspective.ts`)*:
- Il render **flat** di debug è ora `PerspectiveModel.Flat` (l'enum a 3 modelli ha sostituito il vecchio `PERSPECTIVE_ENABLED`; `PERSPECTIVE_Y_FORESHORTEN`/`_Y_SCALE` non esistono più).
- ⚠️ **Perché il bordo basso era TONDO**: in Model A puro il bordo vicino è a `s=1` (zero foreshortening) → disco tondo; lo schiacciamento cresce **solo** con la profondità. Triangolo impossibile: schiacciamento-costante + tassellazione esatta + footprint fisso → ne scegli 2.
- ✅ **Fix = offset dell'arena (`PERSPECTIVE_NEAR_SCALE`, vedi sezione Prospettiva sopra)**: il margine schermo sotto l'arena è pavimento tra camera e bordo vicino → il bordo basso sta già a `sNear<1` → già schiacciato. Geometricamente onesto, **un solo knob**, tiene **tassellazione esatta + pavimento riempito + X invariata** (equivale al "due-fattori" ma senza un tilt arbitrario). Verificato numericamente (tiling err ~1e-14, `unproject` inverte esatto, `sNear=1`=retro-compatibile). Helper `vStackFactor`/`projectYCenter` ora **in uso** (derivano tutto da `projectY`). Calibrato in play **sFar=0.5, sNear=0.7** (≈√0.5 → squash quasi uniforme ~0.7 ovunque).
