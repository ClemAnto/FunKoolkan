...
**Aperti:** (1) **shear della gemma** dalla scala anisotropa (la rotazione del corpo è ora corretta, ±180; il falso "±90" era la linea-raggio del gizmo di debug, risolto) → resta la scelta di design (arte piatta / split base-gemma / no-rotazione); (2) ritirare `InputController` (legacy, cerca ancora `Crossbow` per nome). Modello C (taper mite) resta nel git history (commit `f61282b`) se servisse tornare indietro.
