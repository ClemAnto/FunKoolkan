...
- **Nuovo core CURLING** 🔄: rilevazione aree HOUSE/TEE fatta (greybox, da wirare in editor). **Prossimo**: logica di scoring — rilevare la stone **ferma** sul TEE (overlap `teeArea` + velocità ~0) → convertire le stone **stesso tipo** che toccano `houseArea` in **proiettili** verso Koolkan (stone sul TEE = ultimo proiettile); poi spawner, bombe Make-make, punteggio, overflow. ⏸️ Il vecchio **circuito-di-mana** (glue/poli/ex-magnetismo) è **parcheggiato** come bonus futuro (codice in repo).
